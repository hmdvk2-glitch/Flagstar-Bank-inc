...
import { LedgerEngine } from './modules/ledgerEngine.js';
import { SecurityEngine } from './modules/securityEngine.js';
    ...
    if (state === 'PUBLIC_LOGIN') {
        ...
        if (form) {
            form.addEventListener('submit', async (e) => {
                ...
                if (id && pass) {
                    const user = await SecurityEngine.verifyCredentials(id, pass);
                    if (user) {
                        AuthState.login(user);
                    } else {
                        UIEngine.toast('Invalid credentials or system migrating.', 'error');
                    }
                } else {
